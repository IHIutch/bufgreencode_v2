// Legacy article URLs from the pre-2024 site. Consumed twice:
// 1. Astro's `redirects` config, so `astro dev` and the static build honor them.
// 2. `scripts/write-redirects.mjs`, which emits `dist/_redirects` so Cloudflare
//    serves real 301s instead of Astro's meta-refresh fallback pages.
